...
class StockValuationCategory(models.AbstractModel):
    ...
    def _get_value_exist(self, categ_id, company_id, data={}):
        """
        Compute Total Values
        """
        total_in = total_out = total_int \
            = total_adj = total_begin = total_end = subtotal_cost = 0.0
        for warehouse in self.value_exist[categ_id]:
            ...
            total_end += warehouse.get('ending_qty', 0.0)
...
#         if not data.get('form').get('location_id'):
...
#                                                             'total_end': total_begin + total_in + total_out + total_int + total_adj,
    ...
    def _get_beginning_inventory(self, data, company_id, product_id, current_record):
        ...
        to_date = self.convert_withtimezone(data['form']['end_date'] + ' 23:59:59')
        self._cr.execute(''' 
    ...
    def _get_ending_inventory(self, data, company_id, product_id, current_record):
        """
        Process:
...
#         from_date = self.convert_withtimezone(data['form']['start_date'] + ' 00:00:00')
        ...
        self._cr.execute(''' 
                        ...
                        AS foo
                        GROUP BY id
                    ''', (to_date, tuple(locations), product_id, to_date, tuple(locations), product_id))

        res = self._cr.dictfetchall()
        self.ending_qty = res and res[0].get('qty', 0.0) or 0.0

        current_record.update({'ending_qty': res and res[0].get('qty', 0.0) or 0.0})
        return self.ending_qty
#         if not data.get('form').get('location_id'):

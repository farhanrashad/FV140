...
#             rec.state = 'done'
    ...
    @api.constrains('location','desti_loca_id')
    def check_location_user(self):
        for requisition_rec in self:
            if requisition_rec.create_uid:
                if requisition_rec.create_uid.id != self.env.user.id and self.env.user.id != SUPERUSER_ID:
                    raise Warning('Only user who created this document can change the source and destination field.')
